...
async function submitGoal() {
    ...
    if (goalInpEl.value.length >= 200) {
        goalInpEl.value = "Maximum characters for the goal is 200 words";
        goalInpEl.style.border = "2px solid red";
        goalInpEl.style.outline = "2px solid red";
        setTimeout(() => {
            goalInpEl.value = "";
...
async function deleteGoal(docId, goalElementToRemove) {
    ...
    try {
        await runTransaction(db, async (transaction) => {
            ...
            const goalRef = doc(db, "users", currentUserId, "goals", docId);
...
async function addExistinggoals() {
    ...
    const goalsCollectionRef = collection(db, "users", currentUserId, "goals");
